...
const CommissionGroups = () => {
  ...
  const add = async () => {
    try {
      await postCommission(newName, newRate)
      setNewName("")
      setNewRate(0)
      load()
    } catch (err) {
  ...
  const update = async (id: string, name: string, rate: number) => {
    try {
      await putCommission(id, name, rate)
      load()
    } catch (err) {
            ...
            <Table.Row key={g.id}>
              ...
              <Table.Cell>
                <Input
                  ...
                  onChange={(e) =>
                    setGroups((prev) =>
                      prev.map((p) =>
                        p.id === g.id ? { ...p, percentage: Number(e.target.value) } : p
                      )
                    )
                  }
                />
              ...
              <Table.Cell className="flex gap-2">
                <Button
                  size="small"
                  variant="secondary"
                  onClick={() => update(g.id, g.name, g.percentage)}
                >
                  Guardar
                </Button>
                <Button
